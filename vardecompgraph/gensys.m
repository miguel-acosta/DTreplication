function [G1,C,impact,fmat,fwt,ywt,gev,eu,loose]=gensys(g0,g1,c,psi,pi,div,verbose)
...
% Use qzdiv_ordqz instead in order to use the lapack ordering function
% WYP, 6/14/2011
    [a b q z]=qzdiv_ordqz(div,a,b,q,z);
end
gev=[diag(a) diag(b)];
if zxz
   if (verbose), disp('Coincident zeros.  Indeterminacy and/or nonexistence.'); end;
   eu=[-2;-2];
...
if unique
   ...
   if (verbose), fprintf(1,'Indeterminacy.  %d loose endog errors.\n',nloose); end;
   %disp('Generalized eigenvalues')
   %disp(gev);
   %md=abs(diag(a))>realsmall;
   %ev=diag(md.*diag(a)+(1-md).*diag(b))\ev;
