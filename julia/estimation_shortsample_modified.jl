...
#result = optimize(neglogposterior,θ0vec,LBFGS(), Optim.Options(iterations=200,show_trace=true))
#Theta  = Optim.minimizer(result)
#CSV.write("mode_optim.csv", DataFrame(Theta))
Theta  = vec(convert(Array, CSV.read("mode_optim.csv",allowmissing=:none)))

scale = 0.2;    
VarR = scale*H;         ## Variance of the random walk. 
N     = 100_000;             ## number of iterations before 3e5
Nupdate = 1_000;
Nsave = 10_000;
...
while t <= N
    ...
    if post1 == -Inf
        ...
        t += 1
        POST[:,t] = copy(θ0)
        if t % Nupdate == 0 && t <= Int(N*0.6)
         POSTmini = POST[:,Int(0.2*t)+1:t];
         if minimum(eig(cov(POSTmini'))[1])>0.0
            H = cov(POSTmini');
         end
         accrate = naccept/t;
          if accrate < 0.001
          ...
          VarR = scale*H;
          println(string("You are at iteration ", t, " of ", N, ". Acceptance rate is: ", naccept/t), " Scale:",scale)
       
      #  toc()
      #  tic()
        end
        if t % Nsave == 0
          CSV.write(string("posterior/post", Int(t/Nsave) , ".csv"), DataFrame(POSTmini), header = false)
        end 
    end
        
end

POST = POST[:,Int(N*0.2)+1:end]
